...
export default function MobileBpAnimMobile({
  setAnimPhase,
}: {
  ...
  useEffect(() => {
    ...
    const textTo = { opacity: 1, duration: 0.4, x: 0 }
  ...
  return (
    <>
      <div className="intro-section relative h-[100vh] w-full xs:flex xs:justify-center">
        ...
        <div className="absolute w-[680px] h-[60vh] top-[calc(20vh-40px)] left-[calc(-140px+12vw)] flex flex-row ">
          <MobileLayout02
            className="relative h-[460px] top-[30px] "
            svgRef02={svgRef02}
          />
          <MobileLayout03
            className="relative h-[460px] top-[calc(18vh-20px)] left-[-50px] ml-[calc(12vw)]"
            svgRef03={svgRef03}
          />
        </div>
        <div className="footer absolute bottom-0 h-[80px] w-full z-60"></div>
      </div>
    </>
  )
}
